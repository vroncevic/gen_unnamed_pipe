...
class GenUnnamedPipe(CfgCLI):
    '''
        ...
        It defines:
            ...
            :methods:
                ...
                | __str__ - dunder method for GenUnnamedPipe.
    '''

    GEN_VERBOSE = 'GEN_UNNAMED_PIPE'
    CONFIG = '/conf/gen_unnamed_pipe.cfg'
    LOG = '/log/gen_unnamed_pipe.log'
    LOGO = '/conf/gen_unnamed_pipe.logo'
    ...
    def __init__(self, verbose=False):
        ...
        gen_unnamed_pipe_property = {
            'ats_organization': 'vroncevic',
            'ats_repository': 'gen_unnamed_pipe',
            'ats_name': 'gen_unnamed_pipe',
            'ats_logo_path': '{0}{1}'.format(
                current_dir, GenUnnamedPipe.LOGO
            ),
            'ats_use_github_infrastructure': True
        }
        splash = Splash(gen_unnamed_pipe_property, verbose=verbose)
        base_info = '{0}{1}'.format(current_dir, GenUnnamedPipe.CONFIG)
        CfgCLI.__init__(self, base_info, verbose=verbose)
        verbose_message(GenUnnamedPipe.GEN_VERBOSE, verbose, 'init tool info')
        self.logger = ATSLogger(
            GenUnnamedPipe.GEN_VERBOSE.lower(),
            '{0}{1}'.format(current_dir, GenUnnamedPipe.LOG),
            verbose=verbose
        )
        if self.tool_operational:
            self.add_new_option(
                GenUnnamedPipe.OPS[0], GenUnnamedPipe.OPS[1],
                dest='gen', help='generate option'
            )
            self.add_new_option(
                GenUnnamedPipe.OPS[2], GenUnnamedPipe.OPS[3],
                action='store_true', default=False,
                help='activate verbose mode for generation'
            )
            self.add_new_option(
                GenUnnamedPipe.OPS[4], action='version', version=__version__
            )
    ...
    def process(self, verbose=False):
        ...
        if self.tool_operational:
            ...
            if num_of_args_sys > 1:
                ...
                if operation not in GenUnnamedPipe.OPS:
                    sys.argv.append('-h')
            else:
                sys.argv.append('-h')
            ...
            if not pro_exists:
                if bool(getattr(args, 'gen')):
                    print(
                        '{0} {1} [{2}]'.format(
                            '[{0}]'.format(GenUnnamedPipe.GEN_VERBOSE.lower()),
                    ...
                    if status:
                        success_message(GenUnnamedPipe.GEN_VERBOSE, 'done\n')
                        ...
                        error_message(
                            GenUnnamedPipe.GEN_VERBOSE, 'generation failed'
                        )
                        self.logger.write_log(
                    ...
                    error_message(
                        GenUnnamedPipe.GEN_VERBOSE, 'provide project name'
                    )
                    self.logger.write_log(
                ...
                error_message(
                    GenUnnamedPipe.GEN_VERBOSE, 'project already exist'
                )
                self.logger.write_log(
            ...
            error_message(
                GenUnnamedPipe.GEN_VERBOSE, 'tool is not operational'
            )
            self.logger.write_log(
    ...
    def __str__(self):
        '''
            Dunder method for GenUnnamedPipe.
