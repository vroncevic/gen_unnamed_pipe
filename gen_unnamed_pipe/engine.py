...
Info
    Engine orchestrating the initialization and execution of gen_unnamed_pipe.
'''
...
class GenUnnamedPipe(Base):
    '''
        ...
        It defines:
            ...
            :attributes:
                | _is_initialized - The flag indicating whether the gen_unnamed_pipe engine is initialized.
            ...
            :methods:
                | __init__ - Initializes the gen_unnamed_pipe engine with adapters and services.
                | process - Processes the gen_unnamed_pipe commands.
    '''
    ...
    def __init__(self, bundle: GenUnnamedPipeBundle) -> None:
        '''
            Initializes the gen_unnamed_pipe engine with adapters and services.

            :param bundle: gen_unnamed_pipe bundle containing adapters and services.
            :exceptions: None.
        '''
        ...
        try:
            ...
            self._logger.write_log(INFO, '✅ gen_unnamed_pipe: engine initialized successfully!')

        except (ATSValueError, ATSTypeError) as exc:
            stdout.write(f'❌ gen_unnamed_pipe: {exc}!\n')

        except Exception as exc:
            stdout.write(f'❌ gen_unnamed_pipe unexpected exception: {exc}!\n')

    def process(self) -> bool:
        '''
            Processes the gen_unnamed_pipe commands.
        ...
        try:
            if self.is_initialized():
                ...
                if result.get("returncode") != 0:
                    self._logger.write_log(ERROR, f'❌ gen_unnamed_pipe: {result.get("stderr") or "failed!"}')
                    return False
                else:
                    self._logger.write_log(INFO, '✅ gen_unnamed_pipe: done!')
                    self._logger.write_log(INFO, '✅ gen_unnamed_pipe: exiting successfully!')
                    return True
            else:
                self._logger.write_log(ERROR, '❌ gen_unnamed_pipe: engine not initialized!')
                return False

        except (ATSValueError, ATSTypeError) as exc:
            self._logger.write_log(ERROR, f'❌ gen_unnamed_pipe: {exc}!')
            return False

        except Exception as exc:
            self._logger.write_log(ERROR, f'❌ gen_unnamed_pipe unexpected exception: {exc}!')
            return False
